...
if PGPASSWORD=ona psql -h localhost -U ona -d onaflix -c "SELECT 1" >/dev/null 2>&1; then
    MOVIE_COUNT=$(PGPASSWORD=ona psql -h localhost -U ona -d onaflix -t -c "SELECT COUNT(*) FROM movies" 2>/dev/null | xargs)
